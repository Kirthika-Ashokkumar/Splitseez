...
/**
 * calculateSplit
 * Calculates how much each user owes based on the split_type
 */
function calculateSplit(receipt) {
  ...
  if (total <= 0) throw new Error("Total must be greater than 0. Cannot split a zero bill.");
  ...
  if (receipt.split_type === 'equal') {
    ...
    results = users.map(user => ({
      user,
      amount: Number(amountPerUser.toFixed(2))
    }));
    ...
    if (totalWithExtras <= 0)
      throw new Error("Total including items, tax, and tip must be greater than 0.");
    ...
    results = Object.entries(map).map(([userId, subtotal]) => {
      ...
      return {
        user: userId,
        amount: Number((subtotal + extraShare).toFixed(2))
      };
    });
  }
...
const uploadReceipt = async (req, res) => {
  try {
    ...
    if (total === undefined || total <= 0) return res.status(400).json({ message: 'Total must be greater than 0.' });
    if (split_type === 'item' && (!items || items.length === 0)) {
      return res.status(400).json({ message: 'Items are required for itemized split.' });
    }
...
const updateReceipt = async (req, res) => {
  try {
    ...
    if (total !== undefined) {
      if (total <= 0) return res.status(400).json({ message: 'Total must be greater than 0.' });
      receipt.total = total;
    }
    if (items !== undefined && receipt.split_type === "item") receipt.items = items;
...
const deleteReceipt = async (req, res) => {
  try {
    ...
    await Event.findByIdAndUpdate(eventId, { $pull: { receipts: receiptId } });
...
const getReceipt = async (req, res) => {
  try {
    ...
    const receipt = await Receipt.findById(receiptId)
      ...
      .populate('split_details.user', 'name')  // only include user.name for split
      .populate('items.shared_by', 'name');    // only include user.name for items
...
const getOwedList = async (req, res) => {
  try {
    ...
    const receipt = await Receipt.findById(receiptId)
      .populate('owed.user', 'name');  // populate user name only
