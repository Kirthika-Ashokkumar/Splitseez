...
const ReceiptSchema = new Schema({
  ...
  split_type:
      {type: String, enum: ['equal', 'percent', 'item'], default: 'equal'},
  ...
  owed: [
    {user: {type: Schema.Types.ObjectId, ref: 'User'}, amount: {type: Number}}
  ],
...
ReceiptSchema.pre('save', function(next) {
  if (this.split_type === 'item') {
    ...
    if (Math.abs(calculatedTotal - this.total) > 0.01) {
      return next(new Error(`Total mismatch: provided total (${
          this.total}) does not match calculated total from items + tax + tip (${
          calculatedTotal}).`));
    }
  }
